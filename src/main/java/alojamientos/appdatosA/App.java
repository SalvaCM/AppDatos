	...
	public App() {
		...
		if (descargar)
		{
			...
			for (int i = 0; i < xml.recursoXml.recursos.size(); i++)
			{
				...
				{
					listaAlojamientos = xml.guardarDatosAlojamientos(xml.recursoXml.recursos.get(i).getArchivoXml(), listaAlojamientos);
		...
		{
			// tb puede ser error en la carga asi que hay que dividirlo en dos, una si todos los miembros de array dan false y otra si hay error en la descarga.
